...
// Act
result := auth.CheckPasswordHash(wrongPassword, hash)
...
// Act
token1, err1 := auth.GenerateJWT(userID1, secretKey)
token2, err2 := auth.GenerateJWT(userID2, secretKey)
...
// Act
validatedUserID, err := auth.ValidateJWT(token, secretKey)
...
// Act
userID, err := auth.ValidateJWT(invalidToken, secretKey)
...
// Act
validatedUserID, err := auth.ValidateJWT(token, wrongSecretKey)
...
userID := "test-user-id"
secretKey := "test-secret-key"
tokenString, err := auth.GenerateJWT(userID, secretKey)
if err != nil {
t.Fatalf("Failed to setup test: %v", err)
}
